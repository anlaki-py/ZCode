...
- 本机构建产物直跑（无需代理，server 自带静态托管与 SPA fallback）：`pnpm --filter @zcode/web build` 与 `pnpm --filter @zcode/server build` 之后，从仓库根执行 `PORT=4173 ZCODE_SERVER_HOST=127.0.0.1 ZCODE_WEB_STATIC_ROOT=/home/projects/ZCode/packages/web/dist node packages/server/dist/entry-http.js`，浏览器打开 `http://127.0.0.1:4173/`。如需指定打开目录再加 `ZCODE_SERVER_WORKSPACE=/path`。注意 agent CLI bundle 未构建时 agent 相关功能不可用；完整发行包走 `node scripts/build-zcode.mjs`，但它需要先恢复完整 dev 依赖安装。
